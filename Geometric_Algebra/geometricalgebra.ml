...
let GEOM_MBASIS_SKEWSYM = prove
 (`!i j.
  ...
  ASM_REWRITE_TAC[GEOM_MBASIS_SING] THEN
...
let GEOM_MBASIS_REFL = prove
 (`!i.
    mbasis{i}:real^(N)multivector * mbasis{i} =
...
let G_P_Q_R_WITH_G_N = prove
 (`!p q r i e.
  ...
  let lemma = prove
   (`!i. 2 < i /\ i<= dimindex(:N)
      ==> (mbasis {i-2} * mbasis {i-1} *
          (mbasis {i}:real^(N)multivector)) *
          (mbasis {i-2} * mbasis {i-1} * mbasis {i}) = --mbasis {}`,
    REPEAT STRIP_TAC THEN
    GEN_REWRITE_TAC (LAND_CONV o RAND_CONV o ONCE_DEPTH_CONV)
     [GEOM_MBASIS_SKEWSYM] THEN
    ...
    GEN_REWRITE_TAC 
     (LAND_CONV o RAND_CONV o RAND_CONV o RAND_CONV o ONCE_DEPTH_CONV)
     [GEOM_ASSOC] THEN
    ...
    ASM_SIMP_TAC[
       ARITH_RULE 
         `2 < i /\ i <= dimindex (:N) ==> 1 <= i - 1 /\ i - 1 <= dimindex (:N)`] THEN
    REWRITE_TAC[GEOM_MBASIS_LID; GEOM_MBASIS_REFL; IN_NUMSEG] THEN
    ASM_SIMP_TAC[
       ARITH_RULE
          `2 < i /\ i <= dimindex (:N) ==> 1 <= i - 2 /\ i - 2 <= dimindex (:N)`]) in
  REPEAT STRIP_TAC THEN COND_CASES_TAC THENL
   [ASM_REWRITE_TAC[GEOM_MBASIS_REFL; IN_NUMSEG] THEN ASM_ARITH_TAC; 
    ALL_TAC] THEN
  COND_CASES_TAC THENL
   [SUBGOAL_THEN 
       `2  <  3 * i - 2 * p + r  /\  3 * i - 2 * p + r <= dimindex (:N)` 
     ASSUME_TAC THENL
     [ASM_ARITH_TAC; ALL_TAC] THEN ASM_SIMP_TAC[lemma]; ALL_TAC] THEN
  ASM_REWRITE_TAC[] THEN COND_CASES_TAC THEN
  REWRITE_TAC[GEOM_LADD; GEOM_RADD; GEOM_RZERO] THEN
  ...
  BINOP_TAC THEN REWRITE_TAC[VECTOR_ARITH `a + b = vec 0 <=> b = --a`] THENL
   [REWRITE_TAC[GEOM_MBASIS_REFL; IN_NUMSEG] THEN
    SUBGOAL_THEN 
      `2 < 4 * i - 3 * p - q  /\ 4 * i - 3 * p - q <= dimindex (:N)` 
    ASSUME_TAC THENL
...
let trip_fin_sum_tybij =
  let th = prove
   (`?x.
     x IN 1..(if 1 <= pdimindex(:P) + pdimindex(:Q) + pdimindex(:R)
...
let DIMINDEX_TRIPLE_FINITE_SUM = prove
 (`dimindex(:(P,Q,R)trip_fin_sum) =
  ...
  REWRITE_TAC[REWRITE_RULE[HAS_SIZE] DIMINDEX_HAS_SIZE_TRIPLE_FINITE_SUM] THEN
  ARITH_TAC);;
...
let HAS_SIZE_GEOMALG = prove
 (`(:(P,Q,R)geomalg) HAS_SIZE
    (2 EXP (pdimindex(:P) + pdimindex(:Q) + pdimindex(:R)))`,
...
let MVBASIS_NONZERO = prove
 (`!s.
    s SUBSET 1..pdimindex(:P) + pdimindex(:Q) + pdimindex(:R) 
    ==> ~(mbasis s :real^(P,Q,R)geomalg = vec 0)`,
...
let DOT_MVBASIS_MVBASIS = prove
 (`!s t.
     s SUBSET 1..pdimindex(:P) + pdimindex(:Q) + pdimindex(:R) /\
     t SUBSET 1..pdimindex(:P) + pdimindex(:Q) + pdimindex(:R)
      ==> (mbasis s:real^(P,Q,R)geomalg) dot (mbasis t) = 
...
let DOT_MVBASIS_MVBASIS_UNEQUAL = prove
 ...
 REPEAT STRIP_TAC THEN
 ASM_MESON_TAC[SUM_0; REAL_MUL_RZERO; REAL_MUL_LZERO; COND_ID]);;

let IN_SPAN_IMAGE_MVBASIS = prove
 (`!x:real^(P,Q,R)geomalg s.
     x IN span(IMAGE mbasis s) <=>
     !t. 
       t SUBSET 1..pdimindex(:P) + pdimindex(:Q) + pdimindex(:R) /\ ~(t IN s) 
       ==> x$$t = &0`,
  REPEAT GEN_TAC THEN EQ_TAC THENL
   [SPEC_TAC(`x:real^(P,Q,R)geomalg`,`x:real^(P,Q,R)geomalg`) THEN
  ...
  EXISTS_TAC 
   `(IMAGE mbasis ({t|t SUBSET 1..pdimindex(:P) + pdimindex(:Q) + pdimindex(:R)} INTER s))
    :real^(P,Q,R)geomalg->bool` THEN
  ...
  SIMP_TAC[GEOMALG_EQ; GEOMALG_VSUM_COMPONENT; GEOMALG_MUL_COMPONENT;
           MVBASIS_COMPONENT] THEN
...
let INDEPENDENT_STDMVBASIS_SING = prove
 (`independent 
    {mbasis {i} :real^(P,Q,R)geomalg |
     1 <= i /\ i <= pdimindex(:P) + pdimindex(:Q) + pdimindex(:R)}`,
  MATCH_MP_TAC INDEPENDENT_MONO THEN
  EXISTS_TAC 
   `{mbasis s :real^(P,Q,R)geomalg |
     s SUBSET 1..pdimindex(:P) + pdimindex(:Q) + pdimindex(:R)}` THEN
 ...
 (`!f:real^(P,Q,R)geomalg->real^(P',Q',R')geomalg->real^N g b s.
  ...
  EXISTS_TAC 
   `{mbasis s :real^(P,Q,R)geomalg |
      s SUBSET 1..pdimindex(:P) + pdimindex(:Q) + pdimindex(:R)}` THEN
  EXISTS_TAC 
   `{mbasis t :real^(P',Q',R')geomalg |
     t SUBSET 1..pdimindex(:P') + pdimindex(:Q') + pdimindex(:R')}` THEN
  ASM_REWRITE_TAC[SPAN_MVBASIS; SUBSET_REFL; IN_ELIM_THM] THEN
  ASM_MESON_TAC[]);;
...
let MVBASIS_EXTENSION = prove
 (`!P.
   (!s. s SUBSET 1..pdimindex(:P) + pdimindex(:Q) + pdimindex(:R) ==> P(mbasis s)) /\
  ...
  SUBGOAL_THEN
   `mbasis {i}:real^(P,Q,R)geomalg =
      vsum 
       ...
       (\i'. if i' = i then mbasis {i} else vec 0)`
  SUBST1_TAC THENL
   [REWRITE_TAC[VSUM_DELTA] THEN COND_CASES_TAC THEN
    REWRITE_TAC[MVBASIS_EQ_0] THEN ASM SET_TAC[]; ALL_TAC] THEN
  MATCH_MP_TAC VSUM_EQ THEN REWRITE_TAC[IN_NUMSEG]  THEN
  REPEAT STRIP_TAC THEN
  ASM_CASES_TAC `1 <= pdimindex (:P) + pdimindex (:Q) + pdimindex (:R)` THENL
...
let MULTIVECT_EQ_0 = prove
 (`!x:real^(P, Q, R)trip_fin_sum.
    1 <= pdimindex (:P) + pdimindex (:Q) + pdimindex (:R) 
    ==> (x = vec 0 <=> multivec x = vec 0)`,
  REPEAT STRIP_TAC THEN EQ_TAC THENL 
   [DISCH_TAC THEN ASM_REWRITE_TAC[MULTIVECT_0]; ALL_TAC] THEN
  ...
  REWRITE_TAC[INDEPENDENT_STDMVBASIS_SING; GSYM IN_NUMSEG; SIMPLE_IMAGE] THEN
  SIMP_TAC[FINITE_NUMSEG; FINITE_IMAGE] THEN
  ASSUME_TAC MVBASIS_INJ_SING THEN
  ...
  ONCE_REWRITE_TAC[GSYM CONTRAPOS_THM] THEN REWRITE_TAC[NOT_FORALL_THM] THEN
  REWRITE_TAC[TAUT `~(a==>b) <=> a /\ ~b`] THEN
  STRIP_TAC THEN
  EXISTS_TAC 
   `\v. (x:real^(P, Q, R)trip_fin_sum)$((g:real^(P,Q,R)geomalg->num) v)` THEN
  CONJ_TAC THENL
   [W(MP_TAC o PART_MATCH (lhs o rand) VSUM_IMAGE o lhand o snd) THEN 
    REWRITE_TAC[FINITE_NUMSEG; MVBASIS_INJ_SING; o_DEF] THEN
    DISCH_THEN SUBST1_TAC THEN FIRST_ASSUM(SUBST1_TAC o SYM) THEN
    MATCH_MP_TAC VSUM_EQ THEN ASM_MESON_TAC[]; ALL_TAC] THEN
  POP_ASSUM MP_TAC THEN
  SIMP_TAC[CART_EQ; VEC_COMPONENT; DIMINDEX_TRIPLE_FINITE_SUM] THEN
...
let MULTIVECT_EQ = prove
 (`!x y:real^(P, Q, R)trip_fin_sum.
  ...
  REPEAT STRIP_TAC THEN
  ONCE_REWRITE_TAC[GSYM VECTOR_SUB_EQ; GSYM REAL_SUB_0] THEN
...
let multivectorga = new_definition
 `k multivector (p:real^(P,Q,R)geomalg) <=>
    !s.
     s SUBSET (1..pdimindex(:P) + pdimindex(:Q) + pdimindex(:R)) /\ ~(p$$s = &0)
     ==> s HAS_SIZE k`;;
...
let PRODUCTGA_ASSOCIATIVE = prove
 (`!sgn1 sgn2.
    (!s t u.
      sgn1 t u * sgn2 s (t SYMDIFF u) = sgn2 s t * sgn1 (s SYMDIFF t) u)
  ...
  ASM_SIMP_TAC[IN_ELIM_THM; SYMDIFF_SUBSET; SYMDIFF_ASSOC] THEN
  COND_CASES_TAC THEN ASM_REWRITE_TAC[REAL_MUL_LID; REAL_MUL_RID] THEN
  REWRITE_TAC[REAL_MUL_AC]THEN  ASM_REWRITE_TAC[REAL_MUL_ASSOC] THEN
...
let geomga_mul = new_definition
 `(x:real^(P,Q,R)geomalg) * y =
     Productga (\s t.
       --(&1) pow
        CARD {i,j | i IN 1..pdimindex(:P) + pdimindex(:Q) + pdimindex(:R) /\
                    j IN 1..pdimindex(:P) + pdimindex(:Q) + pdimindex(:R) /\
                    i IN s /\ j IN t /\ i > j} *
       --(&1) pow
        CARD ((pdimindex(:P) + 1..pdimindex(:P) + pdimindex(:Q)) INTER s INTER t) *
       &0 pow 
        CARD ((pdimindex(:P) + pdimindex(:Q) + 1..
                 pdimindex(:P) + pdimindex(:Q) + pdimindex(:R))
              INTER s INTER t))
        x y`;;
...
let GEOMGA_ASSOC = prove
 (`!x y z:real^(P,Q,R)geomalg. x * (y * z) = (x * y) * z`,
  ...
  SIMP_TAC[REAL_ARITH
            `(a:real * b * c) * (d * e * f) =
             (a * d) * (b * e) * (c * f)`] THEN
  REWRITE_TAC[GSYM REAL_POW_ADD; SYMDIFF] THEN BINOP_TAC THENL
   [ALL_TAC;
    BINOP_TAC THENL
     [ALL_TAC;
...
let OUTERGA_ASSOC = prove
 (`!x y z:real^(P,Q,R)geomalg. x outer (y outer z) = (x outer y) outer z`,
  ...
  MATCH_MP_TAC CARD_UNION_LEMMA THEN
  REWRITE_TAC[FINITE_CART_SUBSET_LEMMA1] THEN
...
let PRODUCTGA_MVBASIS = prove
 (`!s t. 
    Productga sgn (mbasis s) (mbasis t) :real^(P,Q,R)geomalg =
...
let PRODUCTGA_MVBASIS_SING = prove
 (`!i j.
    Productga sgn (mbasis{i}) (mbasis{j}) :real^(P,Q,R)geomalg =
 ...
 (`!s t.
     ...
     (if s SUBSET 1..pdimindex(:P) + pdimindex(:Q) + pdimindex(:R) /\
      ...
      then (-- &1 pow CARD {i,j | i IN s /\ j IN t /\ i > j} *
            -- &1 pow 
               CARD ((pdimindex(:P) + 1..pdimindex(:P) + pdimindex(:Q))
                      INTER s INTER t) *
               &0 pow 
               CARD ((pdimindex(:P) + pdimindex(:Q) + 1..
                       pdimindex(:P) + pdimindex(:Q) + pdimindex(:R)) 
...
(`!i j.
  ...
  ASM_REWRITE_TAC[CARD_SING; GSYM ONE; CARD_CLAUSES; real_pow; REAL_MUL_LID;
                  REAL_ARITH `(-- &1) pow 1 = -- &1`] THENL
  ...
  ASM_SIMP_TAC[SYMDIFF; DIFF_EQ_EMPTY; UNION_EMPTY;
    ...
    [GEN_REWRITE_TAC (RAND_CONV o ONCE_DEPTH_CONV) [GSYM VECTOR_MUL_LID];
     GEN_REWRITE_TAC (RAND_CONV o ONCE_DEPTH_CONV) [GSYM VECTOR_MUL_LID] THEN
     REWRITE_TAC[VECTOR_MUL_RNEG; GSYM VECTOR_MUL_LNEG];
     GEN_REWRITE_TAC (RAND_CONV o ONCE_DEPTH_CONV) 
      [GSYM (ISPEC `(mbasis {}):real^(P,Q,R)geomalg` VECTOR_MUL_LZERO)];
     GEN_REWRITE_TAC (RAND_CONV o ONCE_DEPTH_CONV) [GSYM VECTOR_MUL_LID];
     GEN_REWRITE_TAC (RAND_CONV o ONCE_DEPTH_CONV) [GSYM VECTOR_MUL_LID] THEN
     REWRITE_TAC[VECTOR_MUL_RNEG; GSYM VECTOR_MUL_LNEG]] THEN
  AP_THM_TAC THEN AP_TERM_TAC THENL
   ...
   BINOP_TAC THENL
     [GEN_REWRITE_TAC (RAND_CONV o ONCE_DEPTH_CONV) 
       [GSYM (REAL_ARITH `(-- &1) pow 0 = &1`)];
      REWRITE_TAC[GSYM (REAL_ARITH `&0 pow 0 = &1`)];
      GEN_REWRITE_TAC (RAND_CONV o ONCE_DEPTH_CONV) 
       [GSYM (REAL_ARITH `(-- &1) pow 1 = -- &1`)];
      REWRITE_TAC[GSYM (REAL_ARITH `&0 pow 0 = &1`)];
      GEN_REWRITE_TAC (RAND_CONV o ONCE_DEPTH_CONV)
       [GSYM (REAL_ARITH `(-- &1) pow 0 = &1`)];
      GEN_REWRITE_TAC (RAND_CONV o ONCE_DEPTH_CONV)
       [GSYM (REAL_ARITH `&0 pow 1 = &0`)];
      GEN_REWRITE_TAC (RAND_CONV o ONCE_DEPTH_CONV) 
       [GSYM (REAL_ARITH `(-- &1) pow 0 = &1`)];
      REWRITE_TAC[GSYM (REAL_ARITH `&0 pow 0 = &1`)];
      GEN_REWRITE_TAC (RAND_CONV o ONCE_DEPTH_CONV)
    ...
    AP_TERM_TAC THENL
     [SIMP_TAC[FINITE_NUMSEG; FINITE_INTER; CARD_EQ_0];
      SIMP_TAC[FINITE_NUMSEG; FINITE_INTER; CARD_EQ_0];
      GEN_REWRITE_TAC (RAND_CONV o ONCE_DEPTH_CONV) 
       [GSYM (prove(`CARD {j:num} = 1`, REWRITE_TAC[CARD_SING]))] THEN
      AP_TERM_TAC;
      SIMP_TAC[FINITE_NUMSEG; FINITE_INTER; CARD_EQ_0];
      SIMP_TAC[FINITE_NUMSEG; FINITE_INTER; CARD_EQ_0];
      GEN_REWRITE_TAC (RAND_CONV o ONCE_DEPTH_CONV) 
    ...
    REWRITE_TAC[IN_INTER; CARD_SING; IN_NUMSEG; EXTENSION;
                IN_ELIM_THM; NOT_IN_EMPTY; IN_SING] THEN
    ASM_MESON_TAC[NOT_LE; LT_SUC_LE; ADD1; ADD_ASSOC;
                  ARITH_RULE `j:num<=p ==> j< p + q + 1`]);;
...
let INNER_MVBASIS_SING = prove
 (`!i j.
  ...
  COND_CASES_TAC THEN
  ASM_REWRITE_TAC[CONJ_ASSOC; AND_CLAUSES; IN_SING; SET_RULE `~({i} = {})`] THEN
  ASM_CASES_TAC `i:num = j` THENL
   [ALL_TAC; ASM_REWRITE_TAC[SING_SUBSET; IN_SING; VECTOR_MUL_LZERO]] THEN
  SUBGOAL_THEN
    `{i',j' | (((((1 <= i' /\ i' <= (pdimindex (:P) + pdimindex (:Q) + pdimindex (:R))) /\
              1 <= j') /\ j' <= pdimindex (:P) + pdimindex (:Q) + pdimindex (:R)) /\
              i' = i) /\ j' = j) /\ i' > j'}  = {}` 
  ...
  ASM_SIMP_TAC[SYMDIFF; DIFF_EQ_EMPTY; UNION_EMPTY] THENL
    [GEN_REWRITE_TAC (RAND_CONV o ONCE_DEPTH_CONV) [GSYM VECTOR_MUL_LID];
     ...
     REWRITE_TAC[VECTOR_MUL_RNEG; GSYM VECTOR_MUL_LNEG];
     GEN_REWRITE_TAC (RAND_CONV o ONCE_DEPTH_CONV) 
      [GSYM (ISPEC `(mbasis {}):real^(P,Q,R)geomalg` VECTOR_MUL_LZERO)]] THEN
  AP_THM_TAC THEN AP_TERM_TAC THENL
     [GEN_REWRITE_TAC (RAND_CONV o ONCE_DEPTH_CONV)[GSYM REAL_MUL_RID];
      GEN_REWRITE_TAC (RAND_CONV o ONCE_DEPTH_CONV)[GSYM REAL_MUL_RID];
   ...
   BINOP_TAC THENL
     [GEN_REWRITE_TAC (RAND_CONV o ONCE_DEPTH_CONV)
      ...
      GEN_REWRITE_TAC (RAND_CONV o ONCE_DEPTH_CONV)
       [GSYM (REAL_ARITH `(-- &1) pow 0 = &1`)];
      GEN_REWRITE_TAC (RAND_CONV o ONCE_DEPTH_CONV)
       [GSYM (REAL_ARITH `&0 pow 1 = &0`)]] THEN
    AP_TERM_TAC THENL
     [SIMP_TAC[FINITE_NUMSEG; FINITE_INTER; CARD_EQ_0];
      SIMP_TAC[FINITE_NUMSEG; FINITE_INTER; CARD_EQ_0];
      GEN_REWRITE_TAC (RAND_CONV o ONCE_DEPTH_CONV)
      ...
      GEN_REWRITE_TAC (RAND_CONV o ONCE_DEPTH_CONV) 
       [GSYM (prove(`CARD {j:num} = 1`, REWRITE_TAC[CARD_SING]))] THEN
      AP_TERM_TAC] THEN
    REWRITE_TAC[IN_INTER; CARD_SING; IN_NUMSEG; EXTENSION; IN_ELIM_THM;
...
let OUTER_MVBASIS = prove
 (`!s t. 
   (mbasis s) outer (mbasis t) :real^(P,Q,R)geomalg =
    if s SUBSET 1..(pdimindex(:P) + pdimindex(:Q) + pdimindex(:R)) /\
       t SUBSET 1..(pdimindex(:P) + pdimindex(:Q) + pdimindex(:R)) /\
       s INTER t = {}
    then --(&1) pow CARD {i,j | i IN s /\ j IN t /\ i > j} % mbasis(s UNION t)
    else vec 0`,
  ...
  COND_CASES_TAC THEN 
  ASM_SIMP_TAC[SYMDIFF;
               SET_RULE `s INTER t = {} ==> s DIFF t UNION t DIFF s = s UNION t`] THEN
...
let OUTER_MVBASIS_SING = prove
 (`!i j.
    mbasis{i} outer mbasis{j} :real^(P,Q,R)geomalg =
    if i IN 1..(pdimindex(:P) + pdimindex(:Q) + pdimindex(:R)) /\
       j IN 1..(pdimindex(:P) + pdimindex(:Q) + pdimindex(:R)) /\
       ~(i = j)
  ...
  SUBST1_TAC THENL
   [REWRITE_TAC[EXTENSION; FORALL_PAIR_THM; IN_ELIM_PAIR_THM; IN_SING] THEN
    ...
    ASM_MESON_TAC[LT_REFL];
    ALL_TAC] THEN
  ASM_SIMP_TAC[GT; SYMDIFF;
               SET_RULE `~(i = j) ==> ({i} DIFF {j}) UNION ({j} DIFF {i}) = {i,j}`] THEN
  FIRST_ASSUM(SUBST1_TAC o MATCH_MP (ARITH_RULE
   `~(i:num = j) ==> (j < i <=> ~(i < j))`)) THEN
...
let GEOM_OUTER_MVBASIS_EQ = prove
 (`!s t.
  ...
  REPEAT STRIP_TAC THEN
  ASM_REWRITE_TAC[GEOM_MVBASIS; OUTER_MVBASIS; INTER_EMPTY; CARD_CLAUSES;
                  real_pow; REAL_MUL_RID] THEN
  ASM_SIMP_TAC[SYMDIFF;
               SET_RULE `s INTER t = {} ==> s DIFF t UNION t DIFF s = s UNION t`]);;
...
let MVBASIS_INNER_GEOM = prove
 (`!s t.
    (mbasis s) inner (mbasis t) :real^(P,Q,R)geomalg =
    if  ~(s = {}) /\ ~(t = {}) /\ (s SUBSET t \/ t SUBSET s)
    then mbasis s * mbasis t else vec 0`,
  REPEAT STRIP_TAC THEN COND_CASES_TAC THEN
...
(`!s t u.
  ...
  ASM_REWRITE_TAC[OUTER_MVBASIS; GEOM_MVBASIS; OUTERGA_LMUL;
                  GEOMGA_RMUL; UNION_SUBSET] THEN
  ASSUME_TAC 
   (prove(`(s INTER u = {} ==> t INTER u = {} ==> (s SYMDIFF t) INTER u = {}) /\
  ...
  ASM_SIMP_TAC[SYMDIFF_SUBSET; SET_RULE `t INTER u = {} ==> t UNION u = t DIFF u UNION u DIFF t`;
               GSYM SYMDIFF; SYMDIFF_ASSOC; VECTOR_MUL_ASSOC;
               VECTOR_MUL_RZERO; GEOMGA_RZERO; OUTERGA_LZERO] THEN
  AP_THM_TAC THEN AP_TERM_TAC THEN
  REWRITE_TAC[REAL_ARITH `(a * c * d) * b = a:real * b * c * d`;
              REAL_MUL_ASSOC; SYMDIFF] THEN
  BINOP_TAC THENL
    [BINOP_TAC THENL
       [ALL_TAC;
...
(`!s t u.
  ...
  REPEAT STRIP_TAC THEN MAP_EVERY ASM_CASES_TAC
   [`s SUBSET 1..pdimindex(:P) + pdimindex(:Q) + pdimindex(:R)`;
    ...
    `u SUBSET 1..pdimindex(:P) + pdimindex(:Q) + pdimindex(:R)`] THEN
  ASM_CASES_TAC `s:num->bool INTER t = {}` THEN
  ASM_REWRITE_TAC[OUTER_MVBASIS; GEOM_MVBASIS; OUTERGA_RMUL;
                  GEOMGA_LMUL; UNION_SUBSET] THEN
  ASSUME_TAC 
   (prove(`(s INTER u = {} ==> s INTER t = {} ==> s INTER (t SYMDIFF u) = {}) /\
           (s INTER u = {} ==> ~(s INTER t = {}) ==> ~(s INTER (t SYMDIFF u) = {}))`,
      REWRITE_TAC[SYMDIFF] THEN SET_TAC[])) THEN
  ASM_SIMP_TAC[SYMDIFF_SUBSET; SET_RULE `s INTER t = {} ==> s UNION t = s DIFF t UNION t DIFF s`;
               GSYM SYMDIFF; SYMDIFF_ASSOC; VECTOR_MUL_ASSOC; VECTOR_MUL_RZERO;
               GEOMGA_LZERO; OUTERGA_RZERO] THEN
  AP_THM_TAC THEN AP_TERM_TAC THEN
...
let GEOM_MVBASIS_REFL = prove
 (`!i. 
    mbasis{i}:real^(P,Q,R)geomalg * mbasis{i} =
  ...
  COND_CASES_TAC THEN POP_ASSUM MP_TAC THEN
...
let INNER_MVBASIS_REFL = prove
 (`!i.
    mbasis{i}:real^(P,Q,R)geomalg inner mbasis{i} =
    if i IN 1..pdimindex (:P)
    then mbasis {}
    else if i IN pdimindex (:P) + 1..pdimindex (:P) + pdimindex (:Q)
         then --mbasis {}
...
(`!x y:real^(P,Q,R)trip_fin_sum.
  (multivec x) * (multivec y) =
  (multivec x) outer (multivec y) + (multivec x) inner (multivec y)`,
  REWRITE_TAC[multivect] THEN
  SIMP_TAC[FINITE_NUMSEG; BILINEAR_INNERGA; BILINEAR_OUTERGA;
           BILINEAR_GEOMGA ;BILINEAR_VSUM] THEN
  ...
  REWRITE_TAC[GEOMGA_RMUL; GEOMGA_LMUL; OUTERGA_RMUL; OUTERGA_LMUL;
              INNERGA_RMUL; INNERGA_LMUL] THEN
...
let VECTOR_OUTERGA_SKEWSYM = prove
 ( `!x y:real^(P,Q,R)trip_fin_sum. 
     (multivec x) outer (multivec y) = --((multivec y) outer (multivec x))`,
   ...
   GEN_REWRITE_TAC (RAND_CONV o ONCE_DEPTH_CONV)
    [prove(`!P. (\(x,y). P x y) = (\(x,y). P y x) o swappair`,
   ...
   GEN_REWRITE_TAC (LAND_CONV o ONCE_DEPTH_CONV)
    [prove(`!s. {x,y | x IN s /\ y IN s} = IMAGE swappair {x,y | x IN s /\ y IN s}`,
     REWRITE_TAC[EXTENSION; IN_IMAGE; FORALL_PAIR_THM; EXISTS_PAIR_THM;
                 IN_ELIM_PAIR_THM; swappair] THEN 
     MESON_TAC[PAIR_EQ])] THEN
   GEN_REWRITE_TAC(RAND_CONV o ONCE_DEPTH_CONV)[REAL_MUL_SYM] THEN
   MATCH_MP_TAC VSUM_IMAGE THEN
   REWRITE_TAC[GSYM IN_CROSS; GSYM SET_PAIR_THM; IN_ELIM_THM; IN_GSPEC] THEN
   SIMP_TAC[FINITE_NUMSEG; FINITE_CROSS] THEN
...
let VECTOR_INNERGA_REFL = prove
 (`!x:real^(P,Q,R)trip_fin_sum.
     multivec x inner multivec x =
      (sum(1..pdimindex (:P))(\i. x$i * x$i) - 
       sum(1..pdimindex (:Q)) (\i. x$(i+pdimindex (:P)) * x$(i+pdimindex (:P)))) %
      mbasis {}`,
  GEN_TAC THEN
  REWRITE_TAC[multivect] THEN
  SIMP_TAC[FINITE_NUMSEG; BILINEAR_INNERGA; BILINEAR_VSUM] THEN
  ...
  EXISTS_TAC
    `vsum(1..pdimindex(:P) + pdimindex(:Q) + pdimindex(:R))
       (\i. ((x:real^(P,Q,R)trip_fin_sum)$i * x$i) %
       (mbasis {i} inner mbasis {i})):real^(P,Q,R)geomalg` THEN
  CONJ_TAC THENL
   [MATCH_MP_TAC VSUM_EQ THEN
    REWRITE_TAC[INNERGA_RMUL; INNERGA_LMUL; VECTOR_MUL_ASSOC] THEN
    REPEAT STRIP_TAC THEN
    REWRITE_TAC[INNER_MVBASIS_SING] THEN ONCE_REWRITE_TAC[COND_RAND] THEN
    GEN_REWRITE_TAC (LAND_CONV o ONCE_DEPTH_CONV)[EQ_SYM_EQ; VECTOR_MUL_RZERO] THEN
    ASM_SIMP_TAC[VSUM_DELTA]; ALL_TAC] THEN
  REWRITE_TAC[VECTOR_SUB_RDISTRIB; VECTOR_SUB] THEN
  SIMP_TAC[LE_ADDR; VSUM_ADD_SPLIT] THEN BINOP_TAC THENL
   [REWRITE_TAC[GSYM VSUM_RMUL] THEN MATCH_MP_TAC VSUM_EQ THEN
    MESON_TAC[INNER_MVBASIS_REFL];
    ALL_TAC] THEN
  GEN_REWRITE_TAC (LAND_CONV o ONCE_DEPTH_CONV)[ADD_SYM] THEN
  REWRITE_TAC[VSUM_OFFSET] THEN
  REWRITE_TAC[GSYM VECTOR_MUL_RNEG; GSYM VSUM_RMUL] THEN
  ...
  REWRITE_TAC[INNER_MVBASIS_REFL; IN_NUMSEG] THEN CONJ_TAC THEN
  REPEAT STRIP_TAC THEN
...
let VECTOR_GEOMGA_REFL = prove
 (`!x:real^(P,Q,R)trip_fin_sum.
     multivec x * multivec x =
      (sum(1..pdimindex (:P))(\i. x$i * x$i) - sum(1..pdimindex (:Q)) (\i. x$(i+pdimindex (:P)) *
        x$(i+pdimindex (:P)))) % mbasis {}`,
  REWRITE_TAC[VECTOR_GEOM_EQ_OUTER_ADD_INNER; VECTOR_OUTERGA_REFL; 
              VECTOR_INNERGA_REFL; VECTOR_ADD_LID]);;
...
MBASIS_GROUP_CONV 
  `&2 % mbasis{1,2} + &3 % mbasis{2} + &2 % mbasis{1,3} + --(&3) % mbasis{1}
   :real^('4,'1,'1)geomalg`;;
...
let OUTERGA_VECTOR_CONV =
  REWRITE_CONV[REWRITE_RULE[LINEAR_MULTIVECT]
                 (ISPEC `multivec:real^(P,Q,R)trip_fin_sum->real^(P,Q,R)geomalg` linear);
...
let mvinverse = new_definition
  `mvinverse (x:real^(P,Q,R)geomalg) =
    (@x'. x' * x = mbasis {} /\ x * x' = mbasis {})`;;

let MVINVERTIBLE_MVINVERSE = prove
 (`!x:real^(P,Q,R)geomalg.
    mvinvertible x <=>
     mvinverse x * x = mbasis {} /\ x * mvinverse x = mbasis {}`,
...
let MV_LEFT_RIGHT_INVERSE = prove
 (`!x y:real^(P,Q,R)geomalg. x * y = mbasis {} <=> y * x = mbasis {}`,
  ...
  DISCH_THEN(X_CHOOSE_THEN `f':real^(P,Q,R)geomalg->real^(P,Q,R)geomalg`
    STRIP_ASSUME_TAC) THEN
  SUBGOAL_THEN 
    `(f':real^(P,Q,R)geomalg->real^(P,Q,R)geomalg) (x:real^(P,Q,R)geomalg) = mbasis {}`
  MP_TAC THENL
   [GEN_REWRITE_TAC (LAND_CONV o RAND_CONV o ONCE_DEPTH_CONV)[GSYM GEOM_MVBASIS_RID] THEN
    ASM_REWRITE_TAC[]; ALL_TAC] THEN
  GEN_REWRITE_TAC (LAND_CONV o LAND_CONV o RAND_CONV o ONCE_DEPTH_CONV)
   [GSYM GEOM_MVBASIS_LID] THEN
...
let MVINVERTIBLE_VECTOR_EQ = prove
 (`!x:real^(P, Q, R)trip_fin_sum.
  ...
  GEN_TAC THEN
  REWRITE_TAC[is_null; MVINVERTIBLE_LEFT_INVERSE; GSYM VECTOR_GEOMGA_INNER_REFL_EQ] THEN
  EQ_TAC THENL
   [DISCH_TAC THEN
    EXISTS_TAC
     `inv(sum (1..pdimindex (:P)) (\i. x$i * x$i) -
           sum (1..pdimindex (:Q))
              (\i. x$(i + pdimindex (:P)) * x$(i + pdimindex (:P)))) %
               multivec (x:real^(P, Q, R)trip_fin_sum)` THEN
    POP_ASSUM MP_TAC THEN REWRITE_TAC[GEOMGA_LMUL] THEN
    REWRITE_TAC[VECTOR_GEOMGA_REFL] THEN
    REWRITE_TAC[VECTOR_MUL_EQ_0; MVBASIS_EQ_0; EMPTY_SUBSET; VECTOR_MUL_ASSOC] THEN
    SIMP_TAC[REAL_MUL_LINV; VECTOR_MUL_LID]; ALL_TAC] THEN
  DISCH_THEN(X_CHOOSE_TAC `x':real^(P,Q,R)geomalg`) THEN
  DISCH_THEN(MP_TAC o AP_TERM 
   `\y:real^(P,Q,R)geomalg. (x':real^(P,Q,R)geomalg) * x' * y`) THEN
  ASM_REWRITE_TAC[GEOM_ARITH `a * b * c * d = a * (b * c) * d:real^(P,Q,R)geomalg`;
                  GEOM_MVBASIS_LID; GEOMGA_RZERO] THEN
  REWRITE_TAC[MVBASIS_EQ_0; EMPTY_SUBSET]);;

let MVINVERTIBLE_GEOM = prove
  (`!a b:real^(P,Q,R)geomalg.
      mvinvertible a /\ mvinvertible b==> mvinvertible (a * b)`,
...
let MVINVERSE_GEOM = prove
 (`!a b:real^(P,Q,R)geomalg. 
  ...
  REPEAT STRIP_TAC THEN
  ASSUME_TAC
   (SPECL [`a:real^(P,Q,R)geomalg`; `b:real^(P,Q,R)geomalg`]MVINVERTIBLE_GEOM) THEN
...
overload_interface ("outermorphism",
  `outergamorphism:(real^(P,Q,R)trip_fin_sum->real^(S,T,U)trip_fin_sum)->
                    real^(P,Q,R)geomalg->real^(S,T,U)geomalg`);;
...
let SEQITERATE_NUMSEG_IMAGE = prove
 (`!n op m p f:num->real^(P,Q,R)geomalg.
   seqiterate op (IMAGE (\i. i + p) (m..n)) f =
   seqiterate op (m..n) (f o (\i. i + p))`,
  INDUCT_TAC THENL
   [REPEAT GEN_TAC THEN ASM_CASES_TAC `m = 0:num` THENL
    [ALL_TAC;
     POP_ASSUM MP_TAC THEN
     REWRITE_TAC[REWRITE_RULE[LT_NZ]
      (GSYM (SPECL [`m:num`; `0:num`] NUMSEG_EMPTY))] THEN
     DISCH_THEN(SUBST1_TAC)] THEN
     ASM_REWRITE_TAC[NUMSEG_SING; IMAGE_CLAUSES; SEQITERATE_CLAUSES; o_THM];
     ALL_TAC] THEN
  REPEAT GEN_TAC THEN ASM_CASES_TAC `m:num <= SUC n` THENL
   [ASM_SIMP_TAC[GSYM NUMSEG_LREC; IMAGE_CLAUSES; o_THM] THEN
    POP_ASSUM MP_TAC THEN REWRITE_TAC[LE_LT] THEN STRIP_TAC THENL
     [MP_TAC
       (ISPECL 
          [`op:real^(P,Q,R)geomalg->real^(P,Q,R)geomalg->real^(P,Q,R)geomalg`;
           `f:num->real^(P,Q,R)geomalg`; `m + p:num`;
           `(IMAGE (\i. i + p) (m + 1..SUC n)):num->bool`]
            (last(CONJUNCTS SEQITERATE_CLAUSES))) THEN
      MP_TAC
       (ISPECL [`op:real^(P,Q,R)geomalg->real^(P,Q,R)geomalg->real^(P,Q,R)geomalg`;
                `(f o (\i. i + p)):num->real^(P,Q,R)geomalg`; `m:num`;
                `(m + 1..SUC n):num->bool`]
          (last(CONJUNCTS SEQITERATE_CLAUSES))) THEN
      SIMP_TAC[FINITE_NUMSEG; FINITE_IMAGE; IMAGE_EQ_EMPTY; EXTENSION;
               IN_NUMSEG; NOT_IN_EMPTY; IN_IMAGE] THEN
      REPEAT
       (ANTS_TAC THENL
         [CONJ_TAC THENL[REWRITE_TAC[NOT_FORALL_THM] THEN EXISTS_TAC `SUC n:num`; ALL_TAC] THEN
          ASM_ARITH_TAC; ALL_TAC] THEN
        DISCH_THEN(SUBST1_TAC)) THEN
      REWRITE_TAC[o_THM] THEN AP_TERM_TAC THEN
      ASM_REWRITE_TAC[ADD1; NUMSEG_OFFSET_IMAGE; GSYM IMAGE_o; o_DEF; GSYM ADD_ASSOC];
      ALL_TAC] THEN
    ASM_REWRITE_TAC[REWRITE_RULE[ARITH_RULE `n < n + 1`]
     (GSYM (SPECL [`n + 1:num`; `n:num`] NUMSEG_EMPTY))] THEN
    REWRITE_TAC[IMAGE_CLAUSES; SEQITERATE_CLAUSES; o_THM]; ALL_TAC] THEN
  POP_ASSUM MP_TAC THEN
  REWRITE_TAC[REWRITE_RULE[GSYM NOT_LE]
   (GSYM (SPECL [`m:num`; `SUC n:num`] NUMSEG_EMPTY))] THEN
  DISCH_THEN(SUBST1_TAC) THEN
  REWRITE_TAC[IMAGE_CLAUSES; SEQITERATE_CLAUSES; o_THM]);;
 ...
 (`!n m op f:num->real^(P,Q,R)geomalg.
  ...
  REWRITE_TAC[LE_LT] THEN REWRITE_TAC[LT_SUC_LE] THEN STRIP_TAC THENL
   [FIRST_ASSUM(fun th ->REWRITE_TAC[REWRITE_RULE[LE]th]) THEN
    ...
    REWRITE_TAC[REWRITE_RULE[ARITH_RULE `n < n + 1`]
     (GSYM (SPECL [`n + 1:num`; `n:num`]NUMSEG_EMPTY))] THEN
    ASM_REWRITE_TAC[SEQITERATE_CLAUSES];
    MP_TAC
       (ISPECL [`op:real^(P,Q,R)geomalg->real^(P,Q,R)geomalg->real^(P,Q,R)geomalg`;
               `f:num->real^(P,Q,R)geomalg`; `m:num`;
               `(m + 1..SUC (SUC n)):num->bool`]
         (last(CONJUNCTS SEQITERATE_CLAUSES))) THEN
    POP_ASSUM MP_TAC THEN SIMP_TAC[GSYM NUMSEG_LREC] THEN
    REWRITE_TAC[LE_LT] THEN STRIP_TAC THENL
     [MP_TAC
      ...
      REPEAT
       (ANTS_TAC THENL
         [CONJ_TAC THENL[REWRITE_TAC[NOT_FORALL_THM] THEN EXISTS_TAC `SUC n:num`;
          ALL_TAC] THEN
        ASM_ARITH_TAC; ALL_TAC] THEN
        DISCH_THEN(SUBST1_TAC)) THEN
    ...
    ANTS_TAC THENL
     [CONJ_TAC THENL
       [REWRITE_TAC[NOT_FORALL_THM] THEN EXISTS_TAC `SUC (SUC n):num`;
        ALL_TAC] THEN ASM_ARITH_TAC; ALL_TAC] THEN
    DISCH_THEN(SUBST1_TAC) THEN
    ASM_REWRITE_TAC[REWRITE_RULE[ARITH_RULE `n < n + 1`]
     (GSYM (SPECL [`n + 1:num`; `n:num`]NUMSEG_EMPTY))] THEN
    REWRITE_TAC[ADD1; NUMSEG_SING; SEQITERATE_CLAUSES; o_THM];
    ASM_REWRITE_TAC[ADD1; REWRITE_RULE[ARITH_RULE `n < n + 1`]
     (GSYM (SPECL [`n + 1:num`; `n:num`]NUMSEG_EMPTY))] THEN
    ASM_REWRITE_TAC[SEQITERATE_CLAUSES]]);;
 ...
 (`!n m f:num->real^(P,Q,R)geomalg.
  ...
  POP_ASSUM MP_TAC THEN REWRITE_TAC[LE_LT] THEN STRIP_TAC THENL
   [MP_TAC
    ...
    REWRITE_TAC[FINITE_NUMSEG; EXTENSION; IN_NUMSEG; NOT_IN_EMPTY] THEN
    ANTS_TAC THENL
      [CONJ_TAC THENL[REWRITE_TAC[NOT_FORALL_THM] THEN EXISTS_TAC `n:num`; ALL_TAC] THEN
       ASM_ARITH_TAC; ALL_TAC] THEN
...
let SEQITERATE_OUTERGA_RREC =
  GENL[`n:num`; `m:num`]
    (REWRITE_RULE[OUTERGA_ASSOC; NEUTRAL_OUTERGA; OUTER_MVBASIS_LSCALAR]
      (ISPECL[`n:num`; `m:num`;
              `outerga:real^(P,Q,R)geomalg->real^(P,Q,R)geomalg->real^(P,Q,R)geomalg`]
...
let OUTER_SEQITERATE_SYM = prove
 (`!n m x:real^(P, Q, R)trip_fin_sum f.
   ...
   ==> (multivec x) outer seqiterate (outer) (m..n) (multivec o f) =
      (--(&1)) pow (n - m + 1) % 
       (seqiterate (outer) (m..n) (multivec o f) outer (multivec x))`,
  INDUCT_TAC THEN REWRITE_TAC[LE] THEN REPEAT STRIP_TAC THEN
  ASM_SIMP_TAC[NUMSEG_SING; SEQITERATE_CLAUSES; ARITH; o_THM;
               ARITH_RULE `m <= n ==> m <= SUC n`; SEQITERATE_OUTERGA_RREC] THEN
  ...
  REWRITE_TAC[VECTOR_NEG_MINUS1; VECTOR_MUL_ASSOC; OUTERGA_RNEG; VECTOR_MUL_RNEG;
              GSYM VECTOR_MUL_LNEG] THEN
  AP_THM_TAC THEN AP_TERM_TAC THEN
...
let SEQITERATE_OUTERGA_SPLIT3 = prove
 (`!i j m n f:num->real^(P,Q,R)geomalg.
  ...
  FIRST_ASSUM 
   (fun th -> GEN_REWRITE_TAC(RAND_CONV o RAND_CONV o LAND_CONV o DEPTH_CONV)[th]) THEN
...
let SEQITERATE_OUTERGA_NUMSEG_EQ = prove
 (`!n m f:num->real^(P,Q,R)geomalg g.
  ...
  POP_ASSUM MP_TAC THENL
   [SPEC_TAC (`n:num`,`n:num`) THEN SPEC_TAC  (`m:num`,`m:num`) THEN
    MATCH_MP_TAC LE_INDUCT; ALL_TAC] THEN
  MESON_TAC[NUMSEG_EMPTY; NUMSEG_SING; SEQITERATE_CLAUSES; LE_REFL;
            ARITH_RULE `m <= n ==> m <= SUC n`;  SEQITERATE_OUTERGA_RREC]);;
...
let SEQITERATE_IDENTICAL_OUTERGA = prove
 (`!a:num->real^(P, Q, R)trip_fin_sum i j m n.
  ...
  ASM_CASES_TAC `i:num = 0` THENL
   [ASM_REWRITE_TAC[LE] THEN REPEAT STRIP_TAC THEN
    ASM_SIMP_TAC[SEQITERATE_OUTERGA_LREC] THEN
    UNDISCH_TAC `0 < j:num` THEN REWRITE_TAC[GSYM LE_SUC_LT; ARITH] THEN
    REWRITE_TAC[LE_LT] THEN STRIP_TAC THENL
     [ALL_TAC;
      POP_ASSUM (SUBST_ALL_TAC o SYM) THEN
      ASM_SIMP_TAC[SEQITERATE_OUTERGA_LREC; OUTERGA_ASSOC; o_THM;
                   VECTOR_OUTERGA_REFL; OUTERGA_LZERO]] THEN
    MP_TAC
    ...
    SUBGOAL_THEN `j:num = SUC (j - 1)` SUBST_ALL_TAC THENL
     [ASM_ARITH_TAC; ALL_TAC] THEN
    ...
    ASM_REWRITE_TAC[VECTOR_OUTERGA_REFL; OUTERGA_RZERO; VECTOR_MUL_RZERO; OUTERGA_LZERO];
    ALL_TAC] THEN
  POP_ASSUM (MP_TAC o REWRITE_RULE[GSYM LT_NZ]) THEN
  REPEAT STRIP_TAC THEN
  ...
  GEN_REWRITE_TAC(LAND_CONV o RAND_CONV o RAND_CONV o RAND_CONV o ONCE_DEPTH_CONV)
   [OUTERGA_ASSOC] THEN
...
let SEQITERATE_ADD_OUTERGA = prove
 (`!a b c:num->real^(P,Q,R)geomalg k m n.
  ...
  REPEAT STRIP_TAC THEN
  MP_TAC
  (SPECL [`k:num`; `m:num`; `n:num`;
          `(\i. if i = k then a + b else c i):num->real^(P,Q,R)geomalg`]
  SEQITERATE_SPLIT_NUMSEG_OUTERGA) THEN
  MP_TAC
  (SPECL [`k:num`; `m:num`; `n:num`;
          `(\i. if i = k then a else c i):num->real^(P,Q,R)geomalg`]
  SEQITERATE_SPLIT_NUMSEG_OUTERGA) THEN
  MP_TAC
  (SPECL [`k:num`; `m:num`; `n:num`;
          `(\i. if i = k then b else c i):num->real^(P,Q,R)geomalg`]
  SEQITERATE_SPLIT_NUMSEG_OUTERGA) THEN
  ASM_REWRITE_TAC[] THEN REPEAT (DISCH_THEN SUBST1_TAC) THEN
  ASM_CASES_TAC `k:num = 0` THENL
   [UNDISCH_TAC `m <= k:num` THEN ASM_REWRITE_TAC[LE] THEN
    DISCH_THEN SUBST1_TAC THEN
  ...
  SUBGOAL_THEN `k:num = SUC (k - 1)` SUBST_ALL_TAC THENL
   [ASM_ARITH_TAC; ALL_TAC] THEN
  ASM_SIMP_TAC[SEQITERATE_OUTERGA_RREC] THEN
...
(`!a b:num->real^(P,Q,R)geomalg c k m n.
  ...
  MP_TAC
  (SPECL [`k:num`; `m:num`; `n:num`; 
          `(\i. if i = k then c % a else b i):num->real^(P,Q,R)geomalg`]
  SEQITERATE_SPLIT_NUMSEG_OUTERGA) THEN
  MP_TAC
  (SPECL [`k:num`; `m:num`; `n:num`; 
          `(\i. if i = k then a else b i):num->real^(P,Q,R)geomalg`]
  SEQITERATE_SPLIT_NUMSEG_OUTERGA) THEN
  ASM_REWRITE_TAC[] THEN REPEAT (DISCH_THEN SUBST1_TAC) THEN
  ASM_CASES_TAC `k:num = 0` THENL
   [UNDISCH_TAC `m <= k:num` THEN ASM_REWRITE_TAC[LE] THEN
...
(`!a:num->real^(P, Q, R)trip_fin_sum c i j m n.
       m <= i /\ i <= n /\
       m <= j /\ j <= n /\ ~(i = j)
        ==> seqiterate (outer) (m..n)
             (\k. 
              if k = i 
              then (multivec o a) i + c % (multivec o a) j 
              else (multivec o a) k) =
            seqiterate (outer) (m..n) (multivec o a)`,
  SIMP_TAC[SEQITERATE_ADD_OUTERGA; SEQITERATE_MUL_OUTERGA] THEN
  REPEAT STRIP_TAC THEN
  ...
  MATCH_MP_TAC 
   (prove(`x = vec 0:real^N ==> c % x = vec 0:real^N`, SIMP_TAC[VECTOR_MUL_RZERO])) THEN
...
let SEQITERATE_SPAN_OUTERGA = prove
 (`!a:num->real^(P, Q, R)trip_fin_sum i m n x.
     ...
     ==> seqiterate (outer) (m..n) 
          (\k. if k = i then (multivec o a) i + multivec x else (multivec o a) k) =
         seqiterate (outer) (m..n) (multivec o a)`,
...
let SEQITERATE_OUTERGA_VSUM = prove
 (`!m n a:num->real^(P, Q, R)trip_fin_sum.
  ...
  MAP_EVERY SPEC_TAC[(`n:num`, `n:num`); (`m:num`, `m:num`)] THEN
  MATCH_MP_TAC LE_INDUCT THEN CONJ_TAC THENL
   [REWRITE_TAC[NUMSEG_SING; SEQITERATE_CLAUSES; o_THM; ADD_SUB2] THEN
    ...
    MATCH_MP_TAC
     (prove(`c = &0 ==> c % x = vec 0:real^N`, SIMP_TAC[VECTOR_MUL_LZERO])) THEN
  ...
  COND_CASES_TAC THEN
  ASM_SIMP_TAC[ARITH_RULE `m <= n ==> m <= SUC n`; SEQITERATE_OUTERGA_RREC] THEN
  DISCH_TAC THEN
  ...
  REPEAT STRIP_TAC THEN COND_CASES_TAC THEN
  ASM_SIMP_TAC[GEOMALG_VEC_COMPONENT; OUTER_MVBASIS] THEN
  COND_CASES_TAC THEN
  ASM_SIMP_TAC[GEOMALG_MUL_COMPONENT; MVBASIS_COMPONENT;
               GEOMALG_VEC_COMPONENT; REAL_MUL_RZERO] THEN
...
let REVERSION_CONV = 
     SIMP_CONV[REVERSIONGA_LINEAR; REVERSION_MVBASIS; CARD_CLAUSES; IN_INSERT;
               FINITE_EMPTY; FINITE_INSERT; NOT_IN_EMPTY] THENC
     NUM_REDUCE_CONV THENC REAL_RAT_REDUCE_CONV;;
...
(`!x y:real^(P, Q, R)trip_fin_sum.
  ...
  COND_CASES_TAC THENL
   [COND_CASES_TAC THENL
    [ASM_SIMP_TAC[GEOMALG_MUL_COMPONENT; MVBASIS_COMPONENT] THEN
     COND_CASES_TAC THENL
      [ASM_SIMP_TAC[CARD_CLAUSES; IN_INSERT; FINITE_EMPTY;
...
(`!s. 
    reversion (mbasis s):real^(P, Q, R)geomalg =
  ...
  ONCE_REWRITE_TAC
   [SIMP_RULE
     [REAL_POW_NZ; REAL_ARITH `~(--(&1) = &0)`]
      (SPEC `--(&1) pow m` 
         (REAL_FIELD `!z:real. ~(z = &0) ==> (x = y <=> x * z = y * z)`))] THEN
...
let REVERSION_OUTERGA = prove
 (`!x y:real^(P, Q, R)geomalg.
  ...
  REWRITE_TAC[bilinear; linear; OUTERGA_RADD; OUTERGA_LADD; OUTERGA_LMUL; 
              OUTERGA_RMUL; REVERSIONGA_LINEAR; REVERSION_MBASIS] THEN
  REWRITE_TAC[reversionga; OUTER_MVBASIS] THEN
  SIMP_TAC[GEOMALG_EQ; GEOMALG_BETA; GSYM GEOMALG_MUL_COMPONENT] THEN
  REPEAT STRIP_TAC THEN COND_CASES_TAC THEN
  ASM_REWRITE_TAC[INTER_COMM; VECTOR_MUL_ASSOC; VECTOR_MUL_RZERO] THEN
  ASM_SIMP_TAC[GEOMALG_MUL_COMPONENT; MVBASIS_COMPONENT] THEN
  COND_CASES_TAC THEN
  ...
  ONCE_REWRITE_TAC[GSYM (REWRITE_RULE[ARITH] 
                            (SPECL [`m:num`; `n:num`; `2`] EQ_MULT_RCANCEL))] THEN
  ...
  REWRITE_TAC[LEFT_ADD_DISTRIB; RIGHT_ADD_DISTRIB;
              LEFT_SUB_DISTRIB; RIGHT_SUB_DISTRIB] THEN
  MATCH_MP_TAC
   (ARITH_RULE
     `c <= a /\ f <= e /\ (b:num) + d = g + h ==>
       (a + b) - c + (d + e) - f =
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC FINITE_PRODUCT THEN
    ASM_MESON_TAC[FINITE_NUMSEG; FINITE_SUBSET]; ALL_TAC] THEN
  REWRITE_TAC[EXTENSION; IN_INTER; IN_UNION; NOT_IN_EMPTY;
              FORALL_PAIR_THM; IN_ELIM_PAIR_THM] THEN
...
(`!a:num->real^(P, Q, R)trip_fin_sum k.
  ...
  GEN_TAC THEN INDUCT_TAC THENL
   [REWRITE_TAC[REWRITE_RULE[ARITH_RULE `0 < 1`]
     (GSYM (SPECL [`1:num`; `0:num`]NUMSEG_EMPTY))] THEN
...
let MVINVERTIBLE_VERSOR = prove
 (`!x:real^(P,Q,R)geomalg. is_versor x ==> mvinvertible x`,
  ...
  INDUCT_TAC THENL
   [REWRITE_TAC[REWRITE_RULE[ARITH_RULE `0 < 1`]
                              (GSYM (SPECL [`1:num`; `0:num`]NUMSEG_EMPTY));
                NOT_IN_EMPTY] THEN
    REWRITE_TAC[SEQITERATE_CLAUSES; o_THM; NEUTRAL_GEOMGA; MVINVERTIBLE_LEFT_INVERSE] THEN
    MESON_TAC[GEOM_MVBASIS_RID]; ALL_TAC] THEN
   DISCH_TAC THEN
   SIMP_TAC[ARITH_RULE `1 <= SUC k:num`; SEQITERATE_GEOMGA_RREC] THEN
   REWRITE_TAC[o_THM] THEN MATCH_MP_TAC MVINVERTIBLE_GEOM THEN
   CONJ_TAC THENL
    [FIRST_ASSUM MATCH_MP_TAC THEN STRIP_TAC THEN STRIP_TAC THEN
     FIRST_ASSUM MATCH_MP_TAC THEN POP_ASSUM MP_TAC THEN
